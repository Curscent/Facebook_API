    ...
    public ResponseEntity<?> update(@PathVariable Long id, @RequestBody Post payload) {
        return repository.findById(id).map(existing -> {
            ...
            if (payload.getImageUrl() != null) {
                ...
                // Allows setting imageUrl to null if payload specifically omits it,
                // but for PUT/PATCH, we generally only update fields present.
                // Assuming null payload.getImageUrl() means no change to existing.
            }
